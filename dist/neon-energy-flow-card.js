...
var NeonEnergyFlowCardEditor = class extends i4 {
  _config = {};
  static styles = i`
    .section {
      margin-bottom: 16px;
...
var NeonEnergyFlowCard = class extends HTMLElement {
  ...
  _getViewportSize() {
    ...
    if (panel) {
      const r5 = panel.getBoundingClientRect();
      return { vw: r5.width, vh: r5.height };
    }
    const root = this.closest("hui-root");
    if (root) {
      const r5 = root.getBoundingClientRect();
      return { vw: r5.width, vh: r5.height };
    }
    const r4 = this._root?.getBoundingClientRect();
    return {
      vw: r4?.width ?? window.innerWidth,
      vh: r4?.height ?? window.innerHeight
    };
  }
  render() {
